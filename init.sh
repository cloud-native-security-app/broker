...
if command -v docker >/dev/null 2>&1; then
  ...
  warn "docker no está disponible — casi toda la verificación de este repo depende de él (testcontainers, docker compose config)"
fi
...
if [ ! -f "docker-compose.yml" ]; then
  warn "docker-compose.yml no existe todavía — scaffolding pendiente (ver feature 'scaffolding')"
elif ! command -v docker >/dev/null 2>&1; then
  warn "docker no disponible — se omite 'docker compose config'"
else
  if docker compose config >/dev/null 2>&1; then
    ok "docker compose config válido"
  else
    fail "docker compose config encontró errores en docker-compose.yml"
    EXIT_CODE=1
  fi
fi

echo ""
echo "── 5. Compilando, probando y documentando el crate de verificación ─────"
...
if [ ! -f "Cargo.toml" ]; then
  ...
  if cargo doc --no-deps 2>&1; then
    ok "cargo doc genera sin errores"
  else
    fail "cargo doc falló"
    EXIT_CODE=1
  fi
fi
