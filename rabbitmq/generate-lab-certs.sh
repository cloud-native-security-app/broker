...
# de este repo (feature `tls`, ver docs/security-scope.md).
#
...
# como material real. El CN/O del subject (`security-app-lab`) deja
# explícito que no es válido para producción. Ver rabbitmq/README.md
# ("TLS (AMQPS)") para qué cambia si se usan certificados reales.
#
...
# server_key.pem debe ser legible por el usuario "rabbitmq" (no-root)
# dentro del contenedor oficial, que no coincide con el UID del host que
# generó el archivo (bind-mount, no un volumen gestionado por Docker) — sin
# esto, RabbitMQ falla al arrancar con
# "ssl_options.keyfile invalid, PEM file does not exist, cannot be read...".
# Aceptable para material exclusivamente de laboratorio, nunca real.
